...
class PartialArrayBool:
    ...
    def _bitwise(self, other: PartialArrayBool, op, iop):
        if isinstance(other, PartialArrayBool):
    ...
    @property
    def count(self):
        if PartialArrayBool._uint8_bit_sum is None:
            PartialArrayBool._uint8_bit_sum = np.unpackbits(np.arange(256, dtype = np.uint8)).reshape(-1, 8).sum(axis = 1)
        return PartialArrayBool._uint8_bit_sum[self._value].sum()