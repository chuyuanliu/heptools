...
class CBRNG(ABC, Generic[_KeyT]):
    ...
    def choice(
        ...
        a : ~numpy.typing.ArrayLike or int
            If an array-like object, a random sample is generated by choosing from the first dimension. If an int, it is used the same way as :func:`numpy.arange`.
...
class Squares(CBRNG[np.uint64]):
    ...
    @staticmethod
    def _round(lr: npt.NDArray, shift: npt.NDArray, buffer: npt.NDArray, last=False):
        lr *= lr
        lr += shift
        t = lr.copy() if last else None
        np.right_shift(lr, _UINT64_32, out=buffer)
        lr <<= _UINT64_32
        lr |= buffer
        return t
    ...
    def bit64(self, counters: npt.NDArray[np.uint64]) -> npt.NDArray[np.uint64]:
        ...
        for i in (y, z, y):
            self._round(x, i, buffer)
        # round 4
        t = self._round(x, z, buffer, last=True)
        del buffer
        # round 5
        x *= x
        x += y
