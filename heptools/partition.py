...
class Partition:
    ...
    def __init__(self, size: int, groups: int | Iterable[int], members: int = None):
        ...
        if isinstance(groups, Iterable):
            self.members, self.groups = np.unique(np.asarray(groups, int), return_counts = True)
        else:
        ...
        if size < np.sum(self.groups * self.members):
            ...
            if len(self.groups) == 1:
                self.count = Partition._count(self.size, self.groups[0], self.members[0])
            else:
    ...
    def combination(self) -> list[npt.NDArray[np.int_]]:
        ...
        for i in range(1, len(self.groups)):
            result.append(Partition._setdiff2d(np.arange(self.size), *result[:i])[:, self._subs[i].combination[0]].reshape((-1, self.groups[i], self.members[i])))
            for j in range(i):
