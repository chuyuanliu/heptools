...
class Partition:
        ...
        if size < np.sum(self.groups * self.members):
            ...
            if len(self.groups) == 1:
                ...
                for i in range(0, len(self.groups)):
                    self._subs.append(
                        Partition(size, self.groups[i], self.members[i]))
    ...
    def combination(self) -> list[npt.NDArray[np.int_]]:
        if self.count == 0:
            return list(np.empty((0, self.groups[i], self.members[i]), dtype=int) for i in range(len(self.groups)))
        result = [Partition._combination(
            self.size, self.groups[0], self.members[0])]
        for i in range(1, len(self.groups)):
            result.append(Partition._setdiff2d(np.arange(self.size), *result[:i])[
                          :, self._subs[i].combination[0]].reshape((-1, self.groups[i], self.members[i])))
            for j in range(i):
    ...
    def _count(size: int, groups: int, members: int) -> int:
        return prod(comb(size - i * members, members) for i in range(groups)) // perm(groups, groups)
    ...
    def _combination(size: int, groups: int, members: int) -> npt.NDArray[np.int_]:
        if members == 1:
            ...
            return Partition._combination(size, members, groups).reshape((-1, groups, members))
        else:
            combs = np.fromiter(combinations(
                np.arange(size), members), dtype=np.dtype((int, members)))
        if groups == 1:
