...
def pair(p1: Array, p2: Array, name: str|Callable[[Array, Array], str] = _multiple_lorentz_vector_check, behavior = behavior) -> Array:
    if isinstance(name, Callable):
        name = name(p1, p2)
    return ak.zip({'_p1': p1, '_p2': p2}, with_name = name, behavior = behavior)

def pair_all(p: Array, pairs: int = 1, name: str|Callable[[Array], str] = _multiple_lorentz_vector_check, behavior = behavior) -> Array:
    if isinstance(name, Callable):
        name = name(p)
    if pairs == 1:
