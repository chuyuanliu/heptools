    ...
    def constituents(self):
        ...
        for p in (self.obj1, self.obj2):
            try:
                ...
                ps[get_shape(p)[-1]].append(ak.unflatten(p,
                                                         bool(len(p)), axis=len(get_shape(p)) - 2))
        for k, v in ps.items():
...
class _PlotLorentzVector(Template):
    n = H(
        (0, 20, ('n', 'Number')), n=ak.num)
    pt = H(
        (100, 0, 500, ('pt', R'$p_{\mathrm{T}}$ [GeV]')))
    mass = H(
        (100, 0, 500, ('mass', R'Mass [GeV]')))
    eta = H(
        (100, -5, 5, ('eta', R'$\eta$')))
    phi = H(
        (60, -np.pi, np.pi, ('phi', R'$\phi$')))
    pz = H(
        (100, -1000, 1000, ('pz', R'$p_{\mathrm{z}}$ [GeV]')))
    energy = H(
        (150, 0, 1500, ('energy', R'Energy [GeV]')))


class _PlotDiLorentzVector(_PlotLorentzVector):
    dr = H(
        (100, 0, 5, ('dr', R'$\Delta R$')))
    dphi = H(
        (60, -np.pi, np.pi, ('dphi', R'$\Delta\phi$')))
    ht = H(
        (100, 0, 1000, ('ht', R'$H_{\mathrm{T}}$ [GeV]')))
