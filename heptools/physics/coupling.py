...
class Formula(ABC):
    ...
    def _scale_xs_components(self, couplings):
        ...
        diagram_scale = np.product(np.power(couplings, diagrams), axis = 1)
        idx = np.stack(np.tril_indices(diagrams.shape[-1]), axis = -1)
        return np.product(diagram_scale[:, idx], axis = -1)
    ...
    def __init__(self, *basis, unit_basis_weight = True):
        ...
        assert basis.shape[0] >= comb(len(self._diagram) + 1, 2)
