...
class Config(metaclass=ConfigMeta):
    ...
    def __print_parameter__(cls, __par, __type, __value):
        args = {} if __value is Undefined or check_type(
            __value, __type) else {'style': 'red'}
        __source = cls.__track_parameter__(__par)
        __extra = cls.__default__.get(__par) is Extra
        __source = Text(' (') + Text(f'{__source}', style='yellow' + (' italic' if __extra else '')) + Text(
            ')') if (__value is not Undefined) or __extra else Text('')
        __type = Text(' : ') + Text(f'{type_name(__type)}',
                                    style='green') if __type is not Any else Text('')
    ...
    def update(cls, *configs: type[Config] | dict):
        for config in configs:
            if config is cls:
                raise ConfigError(
                    f'cannot update <{cls.__name__}> with itself')
    ...
    def report(cls):  # TODO rename __repr__
        ...
        configs = [config.__name__ for config in [
            *getmro(cls)][::-1] + cls.__unified__ if config not in [object, Config]]
