...
class PileupWeight(Variation, EventLevelCorrection):
    ...
    def _corrections(self):
        return {
            k: self.evaluate(
                weights=self._names.get(k, k),
                NumTrueInteractions=("Pileup", "nTrueInt"),
            )
            ...
            if "cferr" in var:
                groups.append(
                    (
                        var,
                        (flavor_gudsb, {}),
                        (flavor_c, {"systematic": self._names.get(var, var)}),
                    )
                )
            else:
                groups.append(
                    (var, (flavor_gudsb, {"systematic": self._names.get(var, var)}))
                )
        return {
            group[0]: self.evaluate(
                "deepJet_shape",
                *group[1:],
                systematic="central",
                flavor="hadronFlavour",
                abseta=lambda x: np.abs(x.eta),
...
class PileupJetIDSF(Variation, ObjectLevelCorrection):
    ...
    def __init__(
        ...
        self.target = (
            lambda x: x[jet],
            lambda x: x[(x.pt < 50) & (x.pt > 20) & (x.genJetIdx > -1)],
        )
        super().__init__(file, variations)
    ...
    def _corrections(self):
        ...
        if wp:
            MCEff = partial(
                self._evaluate_objects,
                _correction="PUJetID_eff",
                workingpoint=wp,
                systematic="MCEff",
            )
            ...
            return {
                k: self.evaluate(
                    ...
                    *group,
                    workingpoint=wp,
                    systematic=self._names.get(k, k),
                )
                for k in self.variations
            }
        return {"": 1}
