...
class BTagSF_Shape(Variation, ObjectLevelCorrection):
    ...
    def _corrections(self):
        ...
        for var in self.variations:
            if 'cferr' in var:
                groups.append((var, (flavor_gudsb, {}), (flavor_c, {
                              'systematic': self._names.get(var, var)})))
            else:
                groups.append(
...
class PileupJetIDSF(Variation, ObjectLevelCorrection):
    ...
    def __init__(self, file: str, variations: list[str] = ..., jet: str = 'Jet', working_point: int = 0, untagged: bool = False):
        ...
        self.target = (lambda x: x[jet], lambda x: x[(
            x.pt < 50) & (x.pt > 20) & (x.genJetIdx > -1)])
        super().__init__(file, variations)
    ...
    def _corrections(self):
        ...
        if wp:
            MCEff = partial(self._evaluate_objects, _correction='PUJetID_eff',
                            workingpoint=wp, systematic='MCEff')
            tagged = seqcall(
                *self.target, lambda x: x[x.puId >= self.working_point])
            untagged = seqcall(
                *self.target, lambda x: x[x.puId < self.working_point])
            group = [(tagged, {})]
            if self.untagged:
