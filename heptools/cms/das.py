...
class DAS:
    ...
    def query(cls, dataset: str):
        ...
        files = {file['logical_file_name']: {
            'path'      : file['logical_file_name'],
            'nevents'   : file['event_count']
        } for file in files}
        # Rucio query
        for replicas in cls.rucio.list_replicas(
            dids = [{'scope': 'cms', 'name': file} for file in files],
            schemes = ['root']):
            files[replicas['name']]['site'] = [site for site, status in replicas['states'].items() if status == 'AVAILABLE']
