...
URL format
--------------
Both the :class:`ConfigParser` and built-in flags `<include>`, `<file>` accept the standard URL as path to files, and the IO is completely handled by :mod:`fsspec`. 
...
.. warning::
    ...
    When using with :class:`ConfigParser`, the final deserialized object (after all fragments) is required to be a dictionary.
